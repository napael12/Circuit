...
function DataGridColumnHeaderInner<TData extends object, TValue>({
  ...
  const headerLabelClassName = cn(
    // em, not rem, so the header scales with the app's S/M/L text-size setting (see data-grid-table.tsx's own note).
    "text-secondary-foreground/80 inline-flex h-full items-center gap-1.5 font-normal [&_svg]:opacity-60 text-[0.85em] leading-[calc(1.125/0.8125)] [&_svg]:size-3.5",
    className
  )
