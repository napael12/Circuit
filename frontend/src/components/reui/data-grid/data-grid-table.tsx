...
function DataGridTableBase({ children }: { children: ReactNode }) {
  ...
  return (
    <table
      ...
      className={cn(
        // Arbitrary em (not text-sm/rem) so this scales with the app's S/M/L
        // text-size setting, which only resizes the root wrapper div's
        // inline font-size (see store/uiPrefs.ts) -- rem units ignore that
        // and stay pinned to the document's own root <html> size instead.
        "text-foreground caption-bottom text-left align-middle text-[0.85em] font-normal rtl:text-right",
  ...
  return (
    <tr>
      <td
        ...
        className="text-muted-foreground py-6 text-center text-[0.85em]"
      >
...
function DataGridTableLoader() {
  ...
  return (
    <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2">
      <div className="text-muted-foreground bg-card rounded-lg flex items-center gap-2 border px-4 py-2 text-[0.85em] leading-none font-medium">
