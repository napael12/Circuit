...
function DataGridScrollArea({
  ...
  const [hasCustomVerticalOverflow, setHasCustomVerticalOverflow] =
    useState(false)
  // base-ui's own ScrollAreaPrimitive.Scrollbar only renders once its Root's
  // internal `hiddenState` flips to "overflowing", which is driven by a
  // ResizeObserver on the *viewport* box -- that box's own size never
  // changes when rows are appended into a viewport that's already been
  // sized by our flex layout, so overflowing content that arrives after
  // mount (the normal case: data loads async, table starts empty) never
  // triggers a re-measure and the native scrollbar silently never appears.
  // These flags are computed independently (below, via the same
  // ResizeObserver+MutationObserver already needed for the custom vertical
  // overlay) and drive `keepMounted` + a plain `hidden` attribute on the
  // native scrollbars instead, bypassing that internal state entirely.
  const [nativeOverflow, setNativeOverflow] = useState({ x: false, y: false })
  ...
  const syncCustomVerticalScrollbar = useCallback(() => {
    ...
    const hasVerticalOverflow =
      showVertical && scrollHeight > viewportHeight + 0.5
    setNativeOverflow((prev) =>
      prev.x === hasHorizontalOverflow && prev.y === hasVerticalOverflow
        ? prev
        : { x: hasHorizontalOverflow, y: hasVerticalOverflow }
    )
  ...
  }, [resetMetrics, showHorizontal, showVertical, usesCustomVerticalScrollbar])
  ...
  return (
    // min-w-0: this div is itself a flex item of whatever wraps
    // DataGridScrollArea, and a flex item's default min-width is its
    // content's intrinsic width -- without this, the table inside would
    // force this div (and the whole flex chain) wider than the actual
    // container instead of ever overflowing/scrolling within it. w-full is
    // the complementary fix on the *preferred* size: without it, this div
    // (having no flex-grow of its own) shrinks to the table's own content
    // width instead of the flex parent's full available width, which in
    // turn makes every width measurement fed to the fill-column/meta.
    // autoSize machinery below see zero free space to redistribute even
    // when the container plainly has more room than the table needs (e.g.
    // right after a datatable-column is hidden).
